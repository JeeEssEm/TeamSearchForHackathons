...
async def cmd_start(message: Message, state: FSMContext):
    ...
    await message.reply(
        "<b>Привет!</b>" + emoji.emojize(":wink:") + "Давай заполним данные для пользователя. Введи свой email:", parse_mode='HTML'
    )
...
async def process_email(message: Message, state: FSMContext):
    ...
    await message.reply("<b>Введи свою фамилию</b>:")
...
async def process_last_name(message: Message, state: FSMContext):
    ...
    await message.reply("<b>Введи свое имя:</b>", parse_mode='HTML')
...
async def process_first_name(message: Message, state: FSMContext):
    ...
    await message.reply("<b>Введи свое отчество:</b>", parse_mode='HTML')
...
async def process_middle_name(message: Message, state: FSMContext):
    ...
    await message.reply("<b>Введи название своего университета:</b>" + emoji.emojize(":school:"), parse_mode='HTML')
...
async def process_university(message: Message, state: FSMContext):
    ...
    await message.reply("<b>Введи свой курс (число):</b>", parse_mode='HTML')
...
async def process_course(message: Message, state: FSMContext):
    if message.text.isdigit():
        ...
        await message.reply("<b>Введи свою группу:</b>", parse_mode='HTML')
...
async def process_group(message: Message, state: FSMContext, bot: Bot):
    ...
    await message.reply("Теперь выбери свою роль:"+ emoji.emojize(":man_technologist:"))
