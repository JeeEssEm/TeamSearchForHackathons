...
async def test_create_edit_team(test_db, drop_db):
    ...
    async with db.session() as session:
        ...
        # редактирование
        await teams_service.edit_team_by_id(
            t.id,
            dtos.CreateTeam(
                title="team 2",
...
async def test_hack_team(test_db, drop_db):
    ...
    await create_entity(models.Role(title="backend"))
    await create_entity(
        models.Hackathon(title="hack 1", start_date=today, end_date=today)
    )
    await create_entity(
        models.Hackathon(title="hack 2", start_date=today, end_date=today)
    )
    ...
    async with db.session() as session:
        ...
        res = await session.execute(
            select(models.Team).where(models.Team.id == team.id)
        )
...
async def test_users_team(test_db, drop_db):
    ...
    async with db.session() as session:
        ...
        user = await session.execute(
            select(models.users_teams).where(models.users_teams.c.user_id == 2)
        )
